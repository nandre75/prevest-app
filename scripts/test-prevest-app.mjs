...
assert(hero.includes('position: absolute') && hero.includes('.hero__iris-ring') && hero.includes('translateY(-50%)'), 'CTA. avatar aligné colonne sans bleed gauche')
assert(hero.includes('--hero-avatar-gap') && hero.includes('0.58 + var(--hero-avatar-gap)'), 'CTA. marge avatar → texte')
assert(!hero.includes('translate(-28%'), 'CTA. pas de débordement hors colonne')
